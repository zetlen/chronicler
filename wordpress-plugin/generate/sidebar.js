 ...
 *   messages + attached rules) and stages them via the shared
 *   generate/session-tags.js (find-or-create terms, editPost({tags})). The
 *   placeholder's Generate flow already auto-applies these on generation
 *   through the same module; this button re-applies on demand, e.g. after a
 *   wp-tag rule is edited or a staged tag is removed.
 *
    ...
    function applyTags() {
      ...
      Promise.all([fetchSession(sessionId), wp.apiFetch({ path: '/chronicler/v1/rules' })])
        .then(function (results) {
          ...
          return tagsLib.stageTags(names).then(function () {
            setTags({
              busy: false,
